...
内部通信协议描述节点内部通信标准，由通信组件自身实现，用于与SDK或算子算法直接通信。
...
向对方发送报文数据，该接口发送目标为 x-ptp-target-node-id 所在节点通信组件。在通信组件中进行路由。
...
该接口会从通信信道中无阻塞读取一次数据，如信道中无数据，则会返回空。
...
该接口会从通信信道中阻塞读取一次数据，如信道中无数据，会一直阻塞等待触发超时返回空。
